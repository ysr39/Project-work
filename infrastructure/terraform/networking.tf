...
resource "aws_security_group" "alb" {
  ...
  description = "Public ALB — allow 80/443 from internet"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "api" {
  name        = "${local.prefix}-sg-api"
  description = "ECS API tasks — allow traffic from ALB only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "RDS PostgreSQL — allow from API only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "redis" {
  ...
  description = "ElastiCache Redis — allow from API only"
  vpc_id      = aws_vpc.main.id
